...
namespace _03.FindBi_ConnectedComponents
{
    class StartUp
    {
        ...
        private static List<int> componentNodes = new List<int>(); 
        ...
        static void Main()
        {
            ...
            Console.WriteLine($"Number of bi-connected components: {children}");
        }
            ...
            foreach (var child in graph[node])
            {
                if (!visited.Contains(child))
                {
                    ...
                    if (lowestPoints[child] >= depths[node])
                    {
                        ...
                        Console.WriteLine(string.Join(" ", componentNodes));
